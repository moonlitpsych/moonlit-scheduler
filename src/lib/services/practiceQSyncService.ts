...
class PracticeQSyncService {
  ...
  async syncPatientAppointments(
    ...
    for (const intakeqAppt of intakeqAppointments) {
      try {
        ...
        // Generate Google Meet link if needed (telehealth appointments without meeting URL)
        if (result.id && result.syncAction !== 'error') {
          await this.ensureGoogleMeetLink(result.id, intakeqAppt, patientId)
        }
  ...
  /**
   * Ensure Google Meet link exists for telehealth appointments
   *
   * This runs AFTER syncing from IntakeQ. Since IntakeQ API doesn't expose
   * Google Meet links, we generate our own for telehealth appointments.
   *
   * Our link runs in PARALLEL with IntakeQ's link:
   * - IntakeQ link: Visible in IntakeQ UI only (generated by Google Calendar integration)
   * - Our link: Stored in database, visible in all dashboards
   */
  private async ensureGoogleMeetLink(
    ...
    try {
      ...
      const isTelehealth = intakeqAppt.PlaceOfService === '02' ||
                          intakeqAppt.PlaceOfService === '10' ||
                          intakeqAppt.LocationName?.toLowerCase().includes('telehealth')
      ...
      // Check if appointment already has a meeting URL
      const { data: appointment, error: fetchError } = await supabaseAdmin
        .from('appointments')
        .select('meeting_url, start_time, provider_id')
        .eq('id', appointmentId)
        .single()

      if (fetchError || !appointment) {
        console.warn(`⚠️ [Google Meet] Could not fetch appointment ${appointmentId}:`, fetchError?.message)
        return
      }

      if (appointment.meeting_url) {
        // Already has meeting URL, skip
        return
      }

      console.log(`🔗 [Google Meet] Generating link for telehealth appointment ${appointmentId}`)
      ...
      const patientName = patient ? `${patient.first_name} ${patient.last_name}` : intakeqAppt.ClientName

      // Get provider name (already have from IntakeQ)
      const providerName = intakeqAppt.PractitionerName
      ...
      if (meetingUrl) {
        console.log(`✅ [Google Meet] Generated link for ${appointmentId}: ${meetingUrl}`)
        ...
        const { error: updateError } = await supabaseAdmin
          ...
          .update({
            meeting_url: meetingUrl,
            location_info: {
              ...(typeof appointment === 'object' && 'location_info' in appointment ? appointment.location_info : {}),
              locationType: 'telehealth',
              placeOfService: intakeqAppt.PlaceOfService || '02',
        ...
        if (updateError) {
          console.error(`⚠️ [Google Meet] Failed to save meeting URL for ${appointmentId}:`, updateError.message)
        }
      } else {
        console.warn(`⚠️ [Google Meet] Generation returned null for ${appointmentId}`)
      }

    } catch (error: any) {
      // Don't fail the sync if Google Meet generation fails
      console.error(`⚠️ [Google Meet] Error generating link for ${appointmentId}:`, error.message)
    }
  }
