...
interface AdminBulkSyncButtonProps {
  onSyncComplete?: () => void
}
...
export default function AdminBulkSyncButton({ onSyncComplete }: AdminBulkSyncButtonProps) {
  ...
  const handleSync = async () => {
    const confirmed = confirm(
      '⚠️ This will sync ALL active patients with PracticeQ.\n\n' +
    ...
    try {
      ...
      const response = await fetch('/api/admin/patients/bulk-sync-all', {
        method: 'POST',
        headers: {
      ...
      console.error('Admin bulk sync error:', err)
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={isSyncing}
        className={`
          flex items-center gap-2 px-4 py-2 rounded-md shadow-sm text-sm font-medium
          transition-all duration-200
          ${isSyncing
            ? 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-300'
            : 'bg-white text-moonlit-brown border border-moonlit-brown hover:bg-moonlit-cream'
          }
        `}
        title="Sync ALL active patients from PracticeQ (may take several minutes)"
      >
        <RefreshCw className={`w-4 h-4 ${isSyncing ? 'animate-spin' : ''}`} />
        {isSyncing ? 'Syncing All Patients...' : 'Sync All Patients'}
      </button>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 font-['Newsreader']">
                {isSyncing ? 'Syncing All Patients...' : result ? 'Sync Complete' : 'Sync Failed'}
              </h2>
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      ...
                      <li>✅ Payer information updated from IntakeQ insurance fields</li>
                      <li>✅ Finance page will show corrected payer assignments</li>
