    ...
    try {
        ...
        const { data: appointmentData, error: appointmentError } = await supabaseAdmin
            ...
            .insert({
                ...
                notes: notes || '',
                status: 'pending_sync'
                // Note: Removed fields that don't exist in appointments table:
                // - duration_minutes (not in schema)
                // - insurance_policy_id (not in schema)
                // - payer_snapshot (not in schema)
                // - member_snapshot (not in schema)
