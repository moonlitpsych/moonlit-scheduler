...
export default function StaffMedicaidDashboard() {
    ...
    const loadCases = async () => {
        try {
            ...
            const approved = await medicaidOverrideService.getApprovedCases(7)
    ...
    const handleDenyCase = async () => {
        ...
        setLoading(true)
        try {
            const success = await medicaidOverrideService.denyCase(
                selectedCase.case_number,
                currentUser?.email || 'staff@trymoonlit.com',
                approvalNotes || 'Does not meet eligibility requirements'
            )
