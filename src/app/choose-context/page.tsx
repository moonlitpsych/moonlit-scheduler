...
import { Settings, UserCheck, ArrowRight, Building2 } from 'lucide-react'
...
export default function ChooseContextPage() {
  ...
  const partnerRole = authContext.availableRoles.find(role => role.role === 'partner')
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FEF8F1] via-[#F6B398]/30 to-[#FEF8F1] flex items-center justify-center px-4">
      <div className="w-full max-w-3xl">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          {partnerRole && (
            <button
              onClick={() => selectContext('partner')}
              disabled={selecting}
              className="group bg-white rounded-2xl shadow-lg p-8 text-left hover:shadow-xl transition-all duration-200 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed border-2 border-transparent hover:border-[#BF9C73]/20"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="w-16 h-16 bg-gradient-to-br from-[#BF9C73] to-[#F6B398] rounded-2xl flex items-center justify-center group-hover:opacity-90 transition-opacity">
              ...
              <p className="text-[#091747]/70 text-sm font-['Newsreader'] font-light mb-4">
                View patient appointments, manage case assignments, and coordinate care with partner organizations.
              </p>
              ...
              {partnerRole.data && (
                <div className="mb-4 p-3 bg-[#FEF8F1]/50 rounded-lg border border-[#BF9C73]/10">
                  ...
                  <p className="text-sm font-medium text-[#091747] font-['Newsreader']">
                    {partnerRole.data.full_name}
                  </p>
                  {partnerRole.data.organization && (
                    <p className="text-xs text-[#091747]/50 font-['Newsreader'] font-light mt-1">
                      {partnerRole.data.organization.name}
                    </p>
                  )}
                </div>
              )}
