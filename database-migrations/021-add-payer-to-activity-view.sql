-- ============================================================================
-- Migration 021: Add Payer Data to Activity View
-- Created: 2025-10-21
-- Description: Enhance v_patient_activity_summary with primary payer info
--
-- Purpose:
-- - Display current payer on file for each patient
-- - Enable filtering by payer (insurance company)
-- - Enable sorting by payer name
-- - Support payer column on all patient roster variants
-- ============================================================================

-- Drop existing view
DROP MATERIALIZED VIEW IF EXISTS v_patient_activity_summary CASCADE;

-- Recreate with payer fields
...
SELECT
  ...
  -- PracticeQ/IntakeQ sync tracking (NEW)
  p.last_intakeq_sync,
...
CREATE INDEX IF NOT EXISTS idx_vpas_payer ON v_patient_activity_summary(primary_payer_id) WHERE primary_payer_id IS NOT NULL;
...
COMMENT ON MATERIALIZED VIEW v_patient_activity_summary IS
  'Fast dashboard view with engagement status, appointment data, provider assignment, organization affiliations, case manager info, payer data, and PracticeQ sync tracking. Refresh with REFRESH MATERIALIZED VIEW CONCURRENTLY.';
...
SELECT
  'Migration 021: Add Payer to Activity View - COMPLETE' as status,
  (SELECT COUNT(*) FROM v_patient_activity_summary) as total_patients,
  (SELECT COUNT(*) FROM v_patient_activity_summary WHERE primary_payer_id IS NOT NULL) as patients_with_payer,
  (SELECT COUNT(DISTINCT primary_payer_id) FROM v_patient_activity_summary WHERE primary_payer_id IS NOT NULL) as unique_payers,
  (SELECT COUNT(*) FROM v_patient_activity_summary WHERE last_intakeq_sync IS NOT NULL) as patients_synced_with_intakeq;
