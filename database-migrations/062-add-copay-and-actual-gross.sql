-- Migration 062: Add Copay and Actual Gross Tracking
-- Purpose: Separate copay from patient_paid and calculate actual gross revenue
-- Date: 2025-10-30
--
-- Context:
--   patient_paid = Full self-pay amount (cash appointments)
--   copay_cents = Patient cost-sharing for insurance visits (from Stax imports)
--   actual_gross_cents = Total revenue received (reimbursement + patient_paid + copay)

-- ============================================================================
-- STEP 1: Update v_appointments_grid with copay and actual_gross
-- ============================================================================
...
SELECT
  ...
  -- NEW: Copay tracking (separate from full patient payment)
  ...
  -- NEW: Actual Gross (total revenue actually received)
  ...
  -- UPDATED: Actual net (ActualGross - ProviderPaid)
  -- This now correctly uses actual_gross instead of just reimbursement
  greatest(
    coalesce(
      coalesce(
        (o_reimbursement_cents.value #>> '{v}')::numeric,
...
-- Add comments
COMMENT ON VIEW public.v_appointments_grid IS 'Finance appointments grid with copay tracking and actual gross revenue calculation';
COMMENT ON COLUMN public.v_appointments_grid.copay_cents IS 'Patient copay for insurance visits (from Stax imports)';
COMMENT ON COLUMN public.v_appointments_grid.actual_gross_cents IS 'Total revenue received: reimbursement + patient_paid + copay';
...
SELECT '✅ Migration 062: Added copay_cents and actual_gross_cents columns' AS status;
