-- ============================================================================
-- Migration 027: Include no-show appointments in previous appointment tracking
-- Created: 2025-10-27
-- Description: Update v_patient_activity_summary to include 'no_show' status
--
-- Purpose:
-- - IntakeQ marks missed appointments as 'Missed' which maps to 'no_show'
-- - These should appear in "Previous Appointment" column as last scheduled encounter
-- - ONLY CHANGE: Line 25 adds 'no_show' to status filter
-- ============================================================================
...
WITH last_seen AS (
  ...
  FROM appointments
  WHERE status IN ('completed', 'kept', 'no_show')  -- ⬅️ UPDATED: Added 'no_show'
...
COMMENT ON MATERIALIZED VIEW v_patient_activity_summary IS
  'Fast dashboard view with engagement status, appointment data, provider assignment, organization affiliations, and case manager info. UPDATED 2025-10-27: Now includes no-show appointments in last_seen. Refresh with REFRESH MATERIALIZED VIEW CONCURRENTLY.';
...
SELECT
  'Migration 027: Include no-show in previous appointments - COMPLETE' as status,
  (SELECT COUNT(*) FROM v_patient_activity_summary) as total_patients,
  (SELECT COUNT(*) FROM v_patient_activity_summary WHERE last_seen_date IS NOT NULL) as patients_with_previous_appt;
