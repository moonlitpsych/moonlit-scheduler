-- ============================================================================
-- Migration 028: Add status field to last_seen in activity view
-- Created: 2025-10-27
-- Description: Add last_appointment_status to distinguish no-show vs completed
--
-- Purpose:
-- - Display visual indicator in UI for no-show vs completed appointments
-- - Only adds 'status' field to the last_seen CTE
-- ============================================================================

-- Drop existing view
DROP MATERIALIZED VIEW IF EXISTS v_patient_activity_summary CASCADE;

-- Recreate with status field added
CREATE MATERIALIZED VIEW public.v_patient_activity_summary AS
WITH last_seen AS (
  -- Most recent completed/kept/no-show appointment per patient
  SELECT DISTINCT ON (patient_id)
    patient_id,
    start_time as last_seen_date,
    provider_id as last_seen_provider_id,
    id as last_appointment_id,
    status as last_appointment_status  -- ⬅️ NEW: Added status field
  FROM appointments
...
SELECT
  ...
  ls.last_appointment_status,  -- ⬅️ NEW: Added status field
  CASE
...
COMMENT ON MATERIALIZED VIEW v_patient_activity_summary IS
  'Fast dashboard view with engagement status, appointment data, provider assignment, organization affiliations, and case manager info. UPDATED 2025-10-27: Includes no-show appointments and last_appointment_status field. Refresh with REFRESH MATERIALIZED VIEW CONCURRENTLY.';
...
SELECT
  'Migration 028: Add last_appointment_status - COMPLETE' as status,
  (SELECT COUNT(*) FROM v_patient_activity_summary) as total_patients,
  (SELECT COUNT(*) FROM v_patient_activity_summary WHERE last_appointment_status = 'no_show') as patients_with_no_show_last_appt,
  (SELECT COUNT(*) FROM v_patient_activity_summary WHERE last_appointment_status IN ('completed', 'kept')) as patients_with_completed_last_appt;
