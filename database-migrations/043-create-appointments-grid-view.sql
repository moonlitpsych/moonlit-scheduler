-- Migration 043: Create v_appointments_grid View
-- Purpose: Unified view for finance appointments page mirroring spreadsheet
-- Date: 2025-10-29

-- ============================================================
-- SECTION 1: Drop existing view if exists
-- ============================================================

drop view if exists public.v_appointments_grid cascade;

-- ============================================================
-- SECTION 2: Create comprehensive grid view
-- ============================================================
...
-- ============================================================
-- SECTION 4: Create helper views for summaries
-- ============================================================
...
select
  ...
  sum(provider_expected_pay_cents) as total_expected_pay_cents,
  sum(provider_paid_cents) filter (where provider_pay_status = 'PAID') as total_paid_cents,
  sum(provider_expected_pay_cents) filter (where provider_pay_status = 'PENDING') as total_pending_cents,
...
group by provider_id, practitioner, provider_title;

comment on view public.v_provider_pay_summary is 'Provider pay summary for pay run creation';
...
select
  ...
  payer,
  count(*) as appointment_count,
  sum(expected_gross_cents) as total_expected_cents,
  sum(reimbursement_cents) as total_reimbursed_cents,
  sum(expected_net_cents) as total_expected_net_cents,
  sum(actual_net_cents) as total_actual_net_cents
from public.v_appointments_grid
where appt_date is not null
group by appt_date, rev_type, payer
order by appt_date desc;

comment on view public.v_revenue_summary is 'Daily revenue summary by payer type';
grant select on public.v_revenue_summary to authenticated;
...
begin
  if not exists (
    select 1 from pg_views where viewname = 'v_appointments_grid'
  ) then
    raise exception 'Migration 043 failed: v_appointments_grid not created';
  end if;

  raise notice 'Migration 043 completed successfully: Appointments grid view created';
end $$;
