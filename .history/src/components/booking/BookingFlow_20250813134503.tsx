    ...
    const handleROISubmitted = async (roiContacts: ROIContact[]) => {
        ...
        try {
            if (!state.selectedTimeSlot || !state.selectedPayer || !state.insuranceInfo) {
                console.error('Missing data before appointment create')
            ...
            const res = await fetch('/api/patient-booking/create-appointment', {
                ...
                body: JSON.stringify({
                    provider_id: state.selectedTimeSlot.provider_id,
                    service_instance_id: state.selectedTimeSlot.service_instance_id,
                    payer_id: state.selectedPayer.id,
                    start_time: state.selectedTimeSlot.start_time, // ISO
                    timezone: 'America/Denver',
                    patient_info: {
                        first_name: state.insuranceInfo.firstName,
                        last_name: state.insuranceInfo.lastName,
                        date_of_birth: state.insuranceInfo.dateOfBirth,
                        phone: state.insuranceInfo.phone,
                        email: state.insuranceInfo.email
                    },
                    insurance_info: {
                        payer_name: state.selectedPayer.name,
                        member_id: state.insuranceInfo.memberId,
                        group_number: state.insuranceInfo.groupNumber
                    },
            ...
            if (!json?.success) {
                console.error('Create appointment failed:', json?.error)
            }

            // Save appointment id if present and show confirmation
            updateState({ appointmentId: json?.appointment_id })
            goToStep('confirmation')
        } catch (e) {
            console.error('Create appointment error:', e)
            goToStep('confirmation') // Still proceed so user isn’t blocked
        }
    }
