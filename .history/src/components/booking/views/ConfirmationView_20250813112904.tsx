...
interface ConfirmationViewProps {
    ...
    onStartOver: () => void
    onChangeAppointment?: () => void // Quick change appointment
    onChangeInsurance?: () => void  // Quick change insurance
}

/**
 * Flexible formatter:
 * - Current shape: { date: 'YYYY-MM-DD', start_time: 'HH:MM' }
 * - Future ISO shape: { start_time: '2025-08-13T16:00:00.000Z' }
 */
function formatSlotDateTime(slot?: TimeSlot) {
    if (!slot) return ''
    const maybeISO = (slot as any).start_time as string | undefined
    const maybeDate = (slot as any).date as string | undefined

    let dt: Date | null = null

    // If we have ISO-like start_time (contains 'T'), use it directly
    if (maybeISO && typeof maybeISO === 'string' && maybeISO.includes('T')) {
        dt = new Date(maybeISO)
    }
    // Otherwise, use { date + 'HH:MM' } shape
    else if (maybeDate && maybeISO) {
        const [hours, minutes] = String(maybeISO).split(':')
...
function ScenarioTitle(s: BookingScenario) {
    switch (s) {
        case 'self': return 'Your Appointment is Confirmed!'
        case 'referral': return 'Referral Appointment Confirmed!'
        case 'case-manager': return 'Patient Appointment Confirmed!'
        default: return 'Appointment Confirmed!'
    }
}

function ScenarioMessage(s: BookingScenario) {
    switch (s) {
        case 'self':
...
export default function ConfirmationView({
    ...
    const handleCopyId = async () => {
        try {
            ...
            // no-op if clipboard perms not granted
        }
    }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#1a2c5b] to-[#2d4a7c]">
            <div className="container mx-auto px-4 py-8">
                ...
                >
                    {appointmentId ? (
                        <div className="rounded-2xl border border-green-300 bg-green-50 px-4 py-3 text-sm text-green-900 shadow-sm">
                            ...
                            <button
                                ...
                                className="ml-3 inline-flex items-center rounded-md border border-green-300 bg-white/70 px-2 py-1 text-xs hover:bg-white"
                            >
                                Copy
                            </button>
                        </div>
                        ...
                        <div className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 shadow-sm">
                            ✅ Appointment confirmed. If you don’t see a confirmation number, this may be a test booking.
                        </div>
                    )}
                </div>
                ...
                <div className="text-white text-center mb-12">
                    ...
                    <h1 className="text-4xl font-bold mb-4">{ScenarioTitle(bookingScenario)}</h1>
                    <p className="text-xl opacity-90 mb-6">{ScenarioMessage(bookingScenario)}</p>

                    {/* Keep your original reference card for visual emphasis */}
                    {appointmentId && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <p className="text-gray-700 mb-2">
                                        {formatSlotDateTime(selectedTimeSlot)}
                                    </p>
                                    {(selectedTimeSlot as any).duration_minutes && (
                                        <p className="text-sm text-gray-600">
                                            Duration: {(selectedTimeSlot as any).duration_minutes} minutes
                                        </p>
                                    )}
                                    {onChangeAppointment && (
                                        <button
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-700">
                                        {(selectedTimeSlot as any).provider_name || 'Moonlit Psychiatry'}
                                    </p>
