...
function generateX12_270(params: {
    ...
    // Build X12 270 segments
    const segments = [
        `ISA*00*          *00*          *ZZ*${UHIN_CONFIG.tradingPartner}*ZZ*${UHIN_CONFIG.receiverID}*${getCurrentX12Date()}*${getCurrentX12Time()}*U*00401*${controlNumber}*0*P*:~`,
        `GS*HS*${UHIN_CONFIG.tradingPartner}*${UHIN_CONFIG.receiverID}*${getCurrentX12Date()}*${getCurrentX12Time()}*${controlNumber}*X*004010X092A1~`,
        `ST*270*0001~`,
        `BHT*0022*13*${traceNumber}*${getCurrentX12Date()}*${getCurrentX12Time()}~`,
        `HL*1**20*1~`,
        `NM1*PR*2*UTAH MEDICAID*****PI*87726~`,
        `HL*2*1*21*1~`,
        `NM1*1P*2*${UHIN_CONFIG.providerName}*****XX*${UHIN_CONFIG.providerNPI}~`,
        `HL*3*2*22*0~`,
    ...
    segments.push(`DTP*291*D8*${getCurrentX12Date()}~`);
...
function createSOAPEnvelope(x12Content: string) {
    // First, try a simpler SOAP envelope without WS-Security
    return `<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:uhin="http://ws.uhin.org/webservices/core">
    <soap:Body>
        <uhin:ProcessX12>
            <uhin:x12Input>${Buffer.from(x12Content).toString('base64')}</uhin:x12Input>
        </uhin:ProcessX12>
    </soap:Body>
</soap:Envelope>`;
}
...
function parseSOAPResponse(soapResponse: string): string {
    try {
        ...
        const faultMatch = soapResponse.match(/<faultstring>(.*?)<\/faultstring>/s);
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (hasCredentials) {
            // REAL UHIN MODE
            console.log('✅ Using REAL UHIN integration');
