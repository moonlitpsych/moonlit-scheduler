...
// Debug: Log credential status AFTER UHIN_CONFIG is defined
...
function generateX12_270(params: {
    ...
    const traceNumber = `${controlNumber}-MOONLIT`;
    ...
    segments.push(`IEA*1*${controlNumber.padStart(9, '0')}~`);
...
// Generate UUID-format payload ID for CORE envelope
...
// Create CORE-compliant SOAP envelope with WS-Security
...
function parseX12_271WithPlan(x12Response: string) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Get dates
        if (elements[0] === 'DTP') {
            if (elements[1] === '291' || elements[1] === '307') {
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (hasCredentials) {
            ...
            console.log('  - Password length:', UHIN_CONFIG.password.length);
        }
        ...
        if (hasCredentials) {
            ...
            try {
                ...
                // Generate a UUID-format PayloadID
                const payloadID = generatePayloadID();
                console.log('📝 PayloadID (UUID):', payloadID);
